...
`luna --attach <pid>` 要在目标进程忙于任意 Lua 代码(甚至死循环)或阻塞在行编辑器时仍能应答,这本质是个并发问题。luna 的解法刻意绕开了线程化运行时:

- **忙碌时**:内核的 SIGINT 计数钩子每约 20 万条指令触发一次,顺带轮询 attach 套接字(钩子在轮询期间挂起、之后复原,嵌套 `pcall` 保证轮询失败不伤用户代码)。没有独立解释线程,只是搭了中断的便车;
- **空闲时**:replxx 吞掉 EINTR 且异步接口对输入线程自身不生效,于是 `SIGUSR1` 处理器只向一条管道写一个字节(async-signal-safe),专职唤醒线程把它转成 `replxx_emulate_key_press(REPLXX_KEY_ENTER)`——合成回车让 `input()` 返回空行,REPL 主循环自然走到轮询点;
- **协议**:单行请求、`OK/ERR ... \30` 帧回复,`pcall` 全程隔离。没有序列化层、没有消息循环——unix 流套接字 + 行协议就够了。

这是"成熟件优先"的又一次体现:计数钩子(SIGINT 已有)、replxx 的模拟按键 API、luasocket 的 unix 传输,全部是既有积木,新写的只有约两百行 Lua 桥接。

## attach:轮询点,而非事件循环

`luna --attach` 让一个进程观测另一个进程的实时状态。它的实现刻意**不引入**事件循环:

- **两个轮询点**。目标进程在 `luna/luna/serve.lua` 暴露一个非阻塞的 `serve.step()`(accept → 读一行 → pcall 求值 → 封帧应答)。调用它的只有两处:REPL 主循环的空闲间隙,和脚本执行期间 C 计数钩子里每约 20 万条指令的一次顺带调用。没有新线程进 Lua,没有 epoll,语义仍是纯同步的;
...
- **协议一行可读**:应答是 `"OK\n"/"ERR\n" + 正文 + "\30\n"` 的文本帧,`\30` 后的换行让行读取器不悬空;客户端错误走 stderr,stdout 保持结果流干净;
