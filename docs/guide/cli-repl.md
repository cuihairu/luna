...
- **`^C` 中断**:正在执行的死循环可被 `^C` 打断,会话不退出,错误消息含 `interrupted`;状态由 `kernel.clear_interrupt()` 复位。行编辑态按 `^C` 则取消**当前输入行**(replxx 自己回显 `^C`),续行中的整块随之丢弃,回到同一编号的提示符——被取消的行不进历史、不占用 `In[n]`。只有空行上的 `^D` 才结束会话。
