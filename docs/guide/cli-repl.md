...
- **`^C` 中断**:正在执行的死循环可被 `^C` 打断,会话不退出,错误消息含 `interrupted`;状态由 `kernel.clear_interrupt()` 复位。行编辑态按 `^C` 则取消**当前输入行**(replxx 自己回显 `^C`),续行中的整块随之丢弃,回到同一编号的提示符——被取消的行不进历史、不占用 `In[n]`。只有空行上的 `^D` 才结束会话。**一次性模式**(`luna script.lua` 与 `luna -e`)没有下一行可回:`^C` 让进程以退出码 **130**(128 + SIGINT)结束,两种模式口径一致,消息照常写给调用者(脚本走 stderr,`-e` 走会话输出)。
- **历史**:输入持久化到 `~/.luna_history`(带时间戳分隔),重启后 `↑` 召回;replxx 的增量检索(`^R` 风格)可用。
- **Tab 补全**:链式表达式逐级解析——全局名、字段、metatable `__index`;`require "…` 行内补 `luna_modules/` 各层的包名、包内子模块与已加载模块;已注册的插件补全源同样参与(异常源被跳过)。候选是**插入文本**而非整行改写:只有末尾那一段被替换,`string.su` 补成 `string.sub(` 不会吃掉 `string.`,`require "jso` 补成 `require "json"` 不会吃掉引号。
- **帮助糖**:`?expr` 与 `expr?` 打印值的签名、文档与示例;`%whos` 列出当前全部全局。
- **打错字提示**:运行时错误若标注 `(global 'x')` 且 `x` 确实未定义,错误下方给出编辑距离最近的全局名建议——`hint: did you mean 'print'?`(候选歧义时报两个)。

## 语法高亮

输入行与回显都按 scintillua 的 Lua 词法器实时着色(LPeg 驱动)。着色只在 TTY 生效,管道/脚本模式下自动降级;`NO_COLOR` / `--no-color` 可显式关闭——**输入行与回显同进同退**,关掉就是整场无色(行编辑器的光标定位转义与颜色无关,仍会照发)。词法器与配色方案都是可插拔的——见[插件开发](/guide/plugins)。
...
- **`In[n]` / `Out[n]`**:每条输入(含多行整块)登记为 `In[n]`,每个非 nil 结果登记为 `Out[n]`,并同步写入 `_`(最近)与 `__`(次近);
...
**目标端怎么被"敲到"**:目标进程有三类轮询点——REPL 空闲时,attach 客户端的一条 `SIGUSR1` 让阻塞的行编辑器返回空行,主循环顺势轮询一次;脚本(哪怕死循环)执行期间,C 内核的计数钩子每约 20 万条指令顺带轮询一次;`require "loop"` 的事件循环进程则由 prepare 钩子每轮顺带轮询。所以无论目标在等输入、在空转还是在事件循环里,attach 都能及时到达。
