...
static int setup_session(void **state)
{
    ...
    if (luaL_dostring(L,
                      ...
                      "package.preload['luna.magic'] = assert(load(__LUNA_MAGIC_SRC, luna_chunkname('magic')))") != LUA_OK) {
    ...
    const char *src = "local repl = assert(load(__LUNA_REPL_SRC, luna_chunkname('repl')))()\n"
                      "S = repl.new()\n";
